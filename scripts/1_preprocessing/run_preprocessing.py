...
# set up directories
RAW_DATA = '..data/raw_data' # directory with the .eeg, .vmrk and .vhdr files
DATA_EPOCH = '../data/epochdata' # directory where our epoched data will be saved
...
def preprocessing(subject_id, block_type):
    raw = mne.io.read_raw_brainvision("{}/S{}/{}{:06d}.vhdr".format(RAW_DATA, subject_id, FILE_PREFIX), preload=True, eog={'EOG1', 'EOG2'})
    ...
    if block_type == 70:
        epochs.save("{}/{}{:06d}-epo_localizer.fif".format(DATA_EPOCH, FILE_PREFIX, subject_id), overwrite=True)
        
    else:
        epochs.save("{}/{}{:06d}-epo.fif".format(DATA_EPOCH, FILE_PREFIX, subject_id), overwrite=True)

# run the preprocessing steps across subjects
block_type = [70, 71]

for block in block_type:
    for subject_id in [10, 43]: # the range of all our subjects, in this case (10, 43)
